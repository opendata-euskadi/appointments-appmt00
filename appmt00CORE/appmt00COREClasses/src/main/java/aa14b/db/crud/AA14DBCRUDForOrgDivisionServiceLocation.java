...
/////////////////////////////////////////////////////////////////////////////////////////
	@Override
	protected CRUDResult<AA14OrgDivisionServiceLocation> doCreateOrUpdateEntity(final SecurityContext securityContext,
												   			  					final AA14OrgDivisionServiceLocation loc,
												   			  					final PersistenceRequestedOperation requestedOp,
												   			  					final ListensToDBEntityPersistenceEvents<AA14OrgDivisionServiceLocation,AA14DBEntityForOrgDivisionServiceLocation> singleUseDBEntityPersistenceEventListener) {
		// Create the schedule
		CRUDResult<AA14OrgDivisionServiceLocation> outCRUDResult = super.doCreateOrUpdateEntity(securityContext,
															  				  					loc,
															  				  					requestedOp,
															  				  					singleUseDBEntityPersistenceEventListener);
		// manually link the schedule to every location
		if (outCRUDResult.hasSucceeded()
		 && CollectionUtils.hasData(loc.getSchedulesRefs())) {
			// Get the created location
			AA14OrgDivisionServiceLocation createdLoc = outCRUDResult.getOrThrow();
			...
			for (AA14ModelObjectRef<AA14ScheduleOID,
				...
				AA14DBEntityForManyToManyBetweenOrgDivisionServiceLocationAndSchedulePrimaryKey manyToManyPK = AA14DBEntityForManyToManyBetweenOrgDivisionServiceLocationAndSchedulePrimaryKey.from(createdLoc.getOid().asString(),dbSch.getOid());
				AA14DBEntityForManyToManyBetweenOrgDivisionServiceLocationAndSchedule manyToMany = _entityManager.find(AA14DBEntityForManyToManyBetweenOrgDivisionServiceLocationAndSchedule.class,
																													   manyToManyPK);
				if (manyToMany == null) {
					manyToMany = new AA14DBEntityForManyToManyBetweenOrgDivisionServiceLocationAndSchedule(createdLoc.getOid().asString(),dbSch.getOid());
...
//		locDBEntity.setOrgDivisionService(dbService);
	...
	@Override
	protected CRUDResult<AA14OrgDivisionServiceLocation> doDelete(final SecurityContext securityContext, 
																  final AA14OrgDivisionServiceLocationOID oid) {
		// delete the location
		CRUDResult<AA14OrgDivisionServiceLocation> outCRUDResult = super.doDelete(securityContext,
																				  oid);
		// manually link the schedule to every location
		if (outCRUDResult.hasSucceeded()) {
			AA14OrgDivisionServiceLocation loc = outCRUDResult.getOrThrow();
			
			if (CollectionUtils.hasData(loc.getSchedulesRefs())) {
				log.debug("un-linking location with oid={} from schedules={}",
						  loc.getOid(),loc.getSchedulesOids());
				for (AA14ModelObjectRef<AA14ScheduleOID,AA14ScheduleID> ref : loc.getSchedulesRefs()) {
					AA14DBEntityForSchedule dbSch = _entityManager.find(AA14DBEntityForSchedule.class,
																		DBPrimaryKeyForModelObjectImpl.from(ref.getOid()));
					if (dbSch == null) {
						log.error("Cannot un-link the location with oid={} to the schedule with oid={}: the location does NOT exists!!!",
								  loc.getOid(),ref.getOid());
						continue;	// skip
					}
					// remove the reference to the location
					// (manually managing the many to many relation)
					AA14DBEntityForManyToManyBetweenOrgDivisionServiceLocationAndSchedulePrimaryKey manyToManyPK = AA14DBEntityForManyToManyBetweenOrgDivisionServiceLocationAndSchedulePrimaryKey.from(loc.getOid().asString(),dbSch.getOid());
					AA14DBEntityForManyToManyBetweenOrgDivisionServiceLocationAndSchedule manyToMany = _entityManager.find(AA14DBEntityForManyToManyBetweenOrgDivisionServiceLocationAndSchedule.class,
																														   manyToManyPK);
					if (manyToMany != null) {
					...
					// BEWARE!!	schedule <-> location it's a MANY-TO-MANY relation so the schedule descriptor 
					//			also contains a reference to the location that MUST be updated
					AA14Schedule sch = _modelObjectsMarshaller.forReading().fromXml(dbSch.getDescriptor(),
																					AA14Schedule.class);
					sch.removeServiceLocationRef(loc.getOid());
					dbSch.setDescriptor(_modelObjectsMarshaller.forWriting().toXml(loc));
					_entityManager.persist(dbSch);
					_entityManager.flush();
